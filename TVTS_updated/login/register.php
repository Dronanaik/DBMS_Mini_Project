    ...
    <div class="container">
        <div class="signup-content">
            ...
            if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit'])) {
                ...
                $email = $_POST['email'];
                $psw = $_POST['psw'];
                $sql = $conn->prepare("INSERT INTO `p_info` (uname,pname,post,post_id,pno,email,psw) VALUES(?,?,?,?,?,?,?)");
                if ($sql->execute([$uname, $pname, $post, $post_id, $pno, $email, $psw]))
                    echo "<div class='success-message'>Signup Successful, Click here for <a href='login.html'>Login</a></div>";
                $conn->close();
            }
            ?>
        </div>
